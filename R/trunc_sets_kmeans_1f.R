...
kmeans_compute_S_1f_iso <- function(X, estimated_k_means, all_T_clusters,
  ...
  for (i in c(1:n)) {
    ...
    j_star_quad <- norm_sq_phi_kmeans_1f(
      X, v_vec, diff_means_feat, v_norm,
      i, init_list[current_j_prime],
      feat, scaledSigRow, scaledSigRow_2_norm
    )
    for (j in c(1:length(init_list))) {
      current_j_quad <- norm_sq_phi_kmeans_1f(
        X, v_vec, diff_means_feat, v_norm,
        i, init_list[j],
        feat, scaledSigRow, scaledSigRow_2_norm
      )
      ...
      curr_interval <- solve_one_ineq_complement_1f(
        curr_quad$quad,
        curr_quad$linear,
        curr_quad$constant
      )
  ...
  if (T_length > 1) {
    for (l in c(1:(T_length - 1))) {
      ...
      for (i in c(1:n)) {
        ...
        k_star_quad <- norm_phi_canonical_kmeans_1f(
          X, last_centroids, diff_means_feat,
        ...
        for (j in c(1:k)) {
          ...
          if (j != current_cl_i) {
            k_current_quad <- norm_phi_canonical_kmeans_1f(
              X, last_centroids, diff_means_feat,
              v_norm, last_cl, j,
              v_vec, i, weighted_v_i_all_k,
              feat, scaledSigRow, scaledSigRow_2_norm
