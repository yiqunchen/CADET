...
inner_product_phi <- function(X, v, i, j) {
  ...
  coef_list <- list(
    "quad" = as.numeric(quad_coef), "linear" = as.numeric(linear_coef),
...
minus_quad_ineq <- function(quad1, quad2) {
  coef_list <- list(
    "quad" = quad1$quad - quad2$quad, "linear" = quad1$linear - quad2$linear,
  ...
  if (T_length > 1) {
    for (l in c(1:(T_length - 1))) {
      ...
      for (i in c(1:n)) {
        ...
        k_star_quad <- norm_phi_canonical_kmeans(
          X, last_centroids, XTv, XTv_norm, dir_XTv, v_norm, last_cl,
          current_cl_i, v_vec, i, weighted_v_i_all_k
        ) # i is the observation
        for (j in c(1:k)) {
          if (j != current_cl_i) {
            k_current_quad <- norm_phi_canonical_kmeans(
              X, last_centroids, XTv, XTv_norm,
              dir_XTv, v_norm, last_cl, j, v_vec, i, weighted_v_i_all_k
            ) # i is the observation
            curr_quad <- minus_quad_ineq(k_star_quad, k_current_quad)
            curr_interval <- solve_one_ineq_complement(
              curr_quad$quad,
              curr_quad$linear,
              curr_quad$constant
            )
...
kmeans_compute_S_genCov <- function(X, estimated_k_means, all_T_clusters,
  ...
  for (i in c(1:n)) {
    ...
    for (j in c(1:length(init_list))) {
      ...
      curr_quad <- minus_quad_ineq(j_star_quad, current_j_quad)
      curr_interval <- solve_one_ineq_complement(
        (Sig_Inv_factor)^2 * curr_quad$quad,
...
compute_S_single <- function(X, hcl, K, k1, k2) {
  ...
  for (i in k1_obs) {
    for (j in k2_obs) {
      ...
      new_intervals <- solve_one_ineq(
        1, 2 * (cross_ij / dist_means - dist_means),
...
compute_S_single_1f <- function(X, hcl, K, k1, k2, feat) {
  ...
  for (i in k1_obs) {
    for (j in k2_obs) {
      ...
      new_intervals <- solve_one_ineq_1f(
        1, 2 * (diff_ij - diff_means),
  ...
  for (i in k1_obs) {
    for (j in other_obs) {
      ...
      new_intervals <- solve_one_ineq_1f(
        squared_prop_k2, 2 * prop_k2 * (diff_ij - prop_k2 * diff_means),
  ...
  for (i in k2_obs) {
    for (j in other_obs) {
      ...
      new_intervals <- solve_one_ineq_1f(
        squared_prop_k1, 2 * prop_k1 * (diff_ij - prop_k1 * diff_means),
