...
In this tutorial, we demonstrate basic use of the `CADET` package. 
...
We first generate data according to $\mathbf{X} \sim \mathcal{MN}_{n\times q}(\boldsymbol{\mu}, \textbf{I}_n, \sigma^2 \textbf{I}_q)$ with $n=150,q=2,\sigma=1,$ and 
\begin{align}
...
cluster_1 <- 1
cluster_2 <- 3
cl_1_2_inference_demo <- kmeans_inference_1f(X, k=3, 1, 2,
                                      feat=1, iso=TRUE,
                                      sig=sig,
...
cluster_1 <- 1
cluster_2 <- 4
cl_1_4_inference_demo <- kmeans_inference_1f(X, k=4, cluster_1, cluster_2,
                                      feat=1, iso=TRUE,
